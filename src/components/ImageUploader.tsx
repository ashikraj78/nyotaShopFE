...
const ImageUploader: React.FC<ImageUploaderProps> = ({ onDrop }) => {
  ...
  const onDropAccepted = useCallback((acceptedFiles: File[]) => {
    ...
    onDrop(acceptedFiles.map((file: File) =>
      Object.assign(file, {
  ...
  return (
    <div
      {...getRootProps()}
      className="border rounded  p-9"
    > 
        {selectedImage && <Image src={selectedImage} width="500" height="300"  alt="Preview" />}
      <input {...getInputProps()} />
      {isDragActive ? <p>Drop the images here ...</p> : (
