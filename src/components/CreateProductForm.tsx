...
const CreateProductForm: React.FC<Props> = ({ setCreateForm }) => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Product creation is failed");
      }
  ...
  return (
    ...
    >
      <div className="my-4">
        <input
          className="buy_inputPlaceholder border-b-2 text-xl p-2 rounded-md  mb-2 shadow   w-1/2"
      ...
      <div className="my-4">
        <input
          type="submit"
          className="borderTemplate shadow px-4 py-2 text-xl rounded-md cursor-pointer w-1/4"
        />
      </div>
    </form>
  );
